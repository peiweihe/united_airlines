...
```
Use the tweets, corpus the tweets text and convert into dfm. Remove stopwords to clean the  Finally we could get a clean dtm.
...
```
```{r,echo=F}
knitr::include_graphics(path='/Users/shenanqi/data/USairlines/topics.png')
```
...
```
```{r,echo=F}
knitr::include_graphics(path='/Users/shenanqi/data/USairlines/Rplot02.png')
```
...
sum(diag(mt)) / sum(mt)
```
The accuracy is 0.5969463.
Then compute precision, recall and FScore per class.
```{r eval = F}
...
```
```{r,echo=F}
knitr::include_graphics(path='/Users/shenanqi/data/USairlines/Rplot01.jpg')
```
